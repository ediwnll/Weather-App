...
const api =(()=>{
    async function getData(data){
        ...
        const processedData = {
            ...
            current: {
              ...
              sunriseTime: addSeconds(fromUnixTime(forecastData.current.sunrise),forecastData.timezone_offset + (new Date().getTimezoneOffset() * 60),),
              sunsetTime: addSeconds(fromUnixTime(forecastData.current.sunset),forecastData.timezone_offset + (new Date().getTimezoneOffset() * 60),),
              ...
              time: addSeconds(new Date(),forecastData.timezone_offset + (new Date().getTimezoneOffset() * 60),),
            },
            daily: [],
            hourly: [],
        }
        ...
        for(let j =0; j< 24; j+=1){
            processedData.hourly[j] ={
                date:addSeconds(fromUnixTime(forecastData.hourly[j].dt),forecastData.timezone_offset),
    ...
    async function getLocData(data, units= 'metric'){
        try{
            const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${data}&appid=38982387bce58058b2661be97bbebcb5`, {mode: 'cors'},)
